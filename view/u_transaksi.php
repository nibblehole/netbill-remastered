...
<ul class="breadcrumb">
  ...
  <li class="active"><?php echo ucfirst($action) ; ?> Data</li>
      ...
      <?php
        include "./inc/config.php";
        $query = $db->prepare("SELECT * from t_transaksi WHERE id_transaksi='$_GET[id]' " );  
        $query->execute();
		$no=1;                    
...
<form class="form-horizontal" method="POST">
  <fieldset>
    ...
    <div class="form-group">
      <label class="col-sm-2 control-label" id="validate">Tanggal Validasi</label>
      <div class="col-sm-3">
...
?>
  ...
  if(isset($_POST['simpan'])){
    ...
    $query=$db->prepare("UPDATE t_transaksi SET tgl_bayar='$_POST[tgl_bayar]', tgl_validasi='$_POST[tgl_validasi]', nominal='$_POST[nominal]', status='$_POST[status]' WHERE id_transaksi='$_POST[id]'");
    $query->execute();
	}else{
      $query=$db->prepare("UPDATE t_transaksi SET tgl_bayar='$_POST[tgl_bayar]', tgl_validasi='$_POST[tgl_validasi]', nominal='$_POST[nominal]' WHERE id_transaksi='$_POST[id]'");
	  $query->execute();
    }
