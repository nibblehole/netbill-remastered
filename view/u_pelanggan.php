...
<ul class="breadcrumb">
  ...
  <li class="active"><?php echo ucfirst($action) ; ?> Data</li>
      ...
      <?php
        include "./inc/config.php";
        $query = $db->prepare("SELECT * from t_pelanggan WHERE id_pelanggan='$_GET[id]' "); 
        $query->execute();
		$no=1;                 
...
?>
  ...
  if(isset($_POST['simpan'])){
    $query=$db->prepare("UPDATE t_pelanggan SET nama='$_POST[nama]', alamat='$_POST[alamat]', no_hp='$_POST[telpon]', email='$_POST[email]', id_paket='$_POST[paket]' WHERE id_pelanggan='$_POST[id]'");
    $query->execute();
